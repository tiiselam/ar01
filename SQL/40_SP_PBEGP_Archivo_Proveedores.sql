...
begin
	...
	select p.VENDORID,'SP,I,'+	---- CAMPO 1 Tipo de Registro, CAMPO 2 Indicador de Template. Uso HSBC
		...
		replace(left(rtrim(m.RESPBLE),25),',','')+',,,,'+ --- CAMPO 12 Condición de IVA, CAMPO 13, CAMPO 14, CAMPO 15
		','+	---- CAMPO 16
		','	--- campo 17
	from tblPBE001 d
	...
	select p.VENDORID,'SP,I,'+	--- campo 1	Tipo de Registro, campo 2	Indicador de Template. Uso HSBC
		case PBE_Estatus when 1 then 'U,' when 2 then 'D,' end+	--- campo 3	Tipo de Acción
		replace(rtrim(substring(p.TXRGNNUM,1,11)),',','')+','+	--- campo 4	CUIT Proveedor
		','+---rtrim(p.VENDORID)+','+	--- ---- campo 5	Razón Social/Nombre Proveedor
		','+---rtrim(p.ADDRESS1+' '+p.ADDRESS2+' '+p.ADDRESS3)+','+		---- campo 6	Domicilio Proveedor
		','+---rtrim(p.CITY)+','+	--- campo 7	Localidad Proveedor
		','+	---- campo 8 Código Postal del Proveedor
		','+	---- campo 9 Provincia Proveedor
		rtrim(left(pbe_sucursal,3))+','+	--- campo 10 Sucursal de Entrega de Cheques
		','+---rtrim(d.PBE_NumIngresos)+','+	---- campo 11 Número de Ingresos Brutos Proveedor
