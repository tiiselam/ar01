...
begin
	...
	select trx.VCHRNMBR,'ID,I,CC,ARS,N,'+     ------- campos 1,2,3,4,5
			replace(rtrim(isnull(left(ch.BANACTNU,10),'')),'/','')+','+		------ campo 6 cuenta emision
	...
	select trx.VCHRNMBR,'RE,I,'+	--- campo 1 campo 2
		...
		ISNULL(RTRIM(SUBSTRING(C.Descripcion,1,CASE WHEN CHARINDEX('-',c.Descripcion,1)=0 THEN 20 ELSE CHARINDEX('-',c.Descripcion,1)-1 END)),'')+','+	--- campo 7 descripcion codigo oficial
		...
		case SUBSTRING(dr.nfRET_Regimen,1,CASE WHEN CHARINDEX('-',DR.nfRET_Regimen,1)=0 THEN 3 ELSE CHARINDEX('-',DR.nfRET_Regimen,1)-1 END) when 'IIBB' then RTRIM(SUBSTRING(DR.nfRET_Descripcion,1,CASE WHEN CHARINDEX('-',dr.nfRET_Descripcion,1)=0 THEN 15 ELSE CHARINDEX('-',dr.nfRET_Descripcion,1)-1 END)) end +','+	--- campo 12 Resolución DGR
